...
#  ----------------------------------------------------------------
  ...
  # num_shows should be aggregated based on number of upcoming shows per venue.3
  # need to query the shows by venue
@app.route('/venues')
def venues():
      all_venues = Venue.query.all()

      #need to create list of venues
      data = []
            ...
            for venue in loc_venues:
                  data.append({
                    ...
                    "num_upcoming_shows": len(
                      #need to make this a Show Query
                      db.session.query(Show).filter(Show.venue.id==1).filter(Show.start_time>datetime.now()).all()
                      )
                  })
            data.append({
              "city": location.city,
...
def search_venues():
      ...
      for results in search_results:
            response["data"].append({
              "id": results.id,
              "name": results.name,
              "num_upcoming_shows": len(results.shows)
            })
      db.session.close()
...
def create_venue_submission():
  ...
  #flash('Venue ' + request.form['name'] + ' was successfully listed!')
  # TODO: on unsuccessful db insert, flash an error instead. # completed above
...
def create_artist_submission():
  try:
    ...
    seeking_description = request.form['name']
